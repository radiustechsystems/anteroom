...
#   3. build hello-app and load it into the cluster
#   4. mint the HMAC key Secret the generate policy clones
#   5. apply manifests/ — policies first, then the demo workload
...
if helm status kyverno -n kyverno >/dev/null 2>&1; then
  ...
  # --wait matters: policies applied before the admission webhooks are up
  # would be accepted but not enforced, and the demo pods would be admitted
  # ungated.
...
kind load docker-image hello-app:local --name "$CLUSTER"
